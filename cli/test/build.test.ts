...
test('build with rollup config', async () => {
  ...
  await run({
    ...
    validationFn: async (files: Map<string, string>) => {
      ...
      // Debug: Log all files generated
      console.log('Generated files:', Array.from(files.keys()));
      
      // Verify import map is generated by default
      ok(files.has('dist/importmap.js') || files.has('dist/importmap.json'), 'Import map should be generated by default after build');
      
      if (files.has('dist/importmap.js')) {
        const importMapJs = files.get('dist/importmap.js')!;
        ok(importMapJs.includes('react'), 'Import map should include react dependency');
      } else if (files.has('dist/importmap.json')) {
        const importMapJson = files.get('dist/importmap.json')!;
        ok(importMapJson.includes('react'), 'Import map should include react dependency');
      }
    }
  });
});
...
test('build with --no-install flag', async () => {
  ...
  await run({
    files: filesBuild,
    commands: ['jspm build --out dist --no-install'],
    validationFn: async (files: Map<string, string>) => {
      // Debug: Log all files generated
      console.log('Generated files with --no-install:', Array.from(files.keys()));
      
      // Verify build completed
      ok(files.has('dist/app.js'), 'Entry point app.js should be built');
      ok(files.has('dist/utils.js'), 'Utils.js should be in the output directory');
      
      // Verify import map is NOT generated when --no-install is used
      ok(!files.has('dist/importmap.js'), 'Import map should not be generated with --no-install flag');
      ok(!files.has('dist/importmap.json'), 'Import map JSON should not be generated with --no-install flag');
    }
  });
});
