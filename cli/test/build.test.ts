...
it('build with rollup config', async () => {
  ...
  await run({
    ...
    validationFn: async (files: Map<string, string>) => {
      ...
      ok(
        files.has('dist/importmap.js') || files.has('dist/importmap.json'),
        'Import map should be generated by default after build'
      );
...
it('build with --hash-entries flag', async () => {
  const filesBuild = await mapDirectory('fixtures/scenario_build_app');
  await run({
    files: filesBuild,
...
it('build with --no-install flag', async () => {
  const filesBuild = await mapDirectory('fixtures/scenario_build_app');
  await run({
    files: filesBuild,
    ...
    validationFn: async (files: Map<string, string>) => {
      ...
      // Verify import map is NOT updated when --no-install is used
      ok(
        !files.has('dist/importmap.js'),
        'Import map should not be generated with --no-install flag'
      );
    }
  });
});
