...
test("deploy with missing package.json version", async () => {
  ...
  try {
    ...
    assert(
      error.toString().includes("failed"),
      "Command should fail with missing version"
    );
  }
});
...
test("deploy with invalid tag", async () => {
  ...
  try {
    ...
    assert(
      error.toString().includes("failed"),
      "Command should fail with invalid tag"
    );
  }
});
...
test("deploy and eject", async () => {
  ...
  files.set(
    "index.js",
    "export const message = 'Hello from deployed package';"
  );
  ...
  await run({
    ...
    commands: [
      `jspm deploy ${deployedPackage} --eject -o importmap.json --dir ${ejectDir}`,
    ],
    validationFn: async (updatedFiles) => {
      // Verify ejected files
      assert(
        updatedFiles.has(`${ejectDir}/index.js`),
        "Should have ejected index.js"
      );
      assert(
        updatedFiles.has(`${ejectDir}/src/utils.js`),
        "Should have ejected src/utils.js"
      );

      // Verify import map was updated
      assert(
        updatedFiles.has("importmap.json"),
        "Import map should exist after ejection"
      );
...
test("deploy watch mode basic test", async () => {
  ...
  try {
    ...
    watchProcess = run({
      files,
      commands: [`jspm deploy --version dev --watch --no-usage`],
      validationFn: async () => {
