...
test("Install with deno environment", async () => {
  await run({
    commands: [
      "jspm install -o importmap.json react@17.0.1 react-dom@17.0.1 -e deno",
    ],
    validationFn: async (files: Map<string, string>) => {
      assert.equal(files.size, 3);
...
test("Install with both deno and browser environments", async () => {
  await run({
    commands: [
      "jspm install -o importmap.json react@17.0.1 react-dom@17.0.1 -e deno,browser",
    ],
    validationFn: async (files: Map<string, string>) => {
      assert.equal(files.size, 3);
...
test("Install using an alias for the package", async () => {
  await run({
    commands: [
      "jspm install -o importmap.json -e production custom=react@17.0.1",
    ],
    validationFn: async (files: Map<string, string>) => {
      assert.equal(files.size, 3);
...
test("Swapping providers using the -p flag", async () => {
  await run({
    commands: [
      "jspm install -o importmap.json -p jsdelivr -e production,browser react@17.0.1",
    ],
    validationFn: async (files: Map<string, string>) => {
      assert.equal(files.size, 3);
...
test("Linking with trailing slash should do subpath installs", async () => {
  await run({
    ...
    validationFn: async (files: Map<string, string>) => {
      ...
      assert.strictEqual(
        map.imports["test-package/helpers"],
        "./helpers/index.js"
      );
    },
  });
