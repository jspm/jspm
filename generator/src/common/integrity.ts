...
export async function getIntegrityNodeLegacy(
  ...
  const hash = (_nodeCrypto || (_nodeCrypto = await (0, eval)('import("node:crypto")'))).createHash(
    'sha384'
  );
  hash.update(buf);
...
export let getIntegrity = async function getIntegrity(
  ...
  const hashBuffer = await crypto.subtle.digest('SHA-384', data as Uint8Array<ArrayBuffer>);
