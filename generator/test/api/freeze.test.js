...
async function checkScenario(scenario) {
  ...
  let mdls = [];
  for (const url of Object.values(map.imports || {})) mdls.push(await parseUrlPkg(url));
  function getVersions(pkg) {
    return mdls.filter(mdl => mdl.pkg.name === pkg).map(mdl => mdl.pkg.version);
  }
  ...
  for (let [pkg, version] of Object.entries(scenario.expect ?? {})) {
    ...
    assert(
      ...
      `freeze scenario "${scenario.name}" expected ${pkg}@${version}, but got [ ${getVersions(
        pkg
      ).join(', ')} ]`
    );
  }
        ...
        scopes: {
          'https://ga.jspm.io/': {
            'lit-html/is-server.js':
              'https://ga.jspm.io/npm:lit-html@2.6.0/development/is-server.js',
