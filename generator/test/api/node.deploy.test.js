...
// ---- Simple Package Test ----
{
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to fetch deployed package ${resolvedUrl}: ${response.status}`
      );
    }
  ...
  if (typeof globalThis.process?.versions?.node === "string") {
    ...
    const tmpDirPath = join(
      tmpdir(),
      `tmp-${Math.round(Math.random() * 100_000)}`
    );
    try {
...
// ---- Mutable Package Test ----
{
  ...
  assert.ok(
    initialDeployResult.packageUrl,
    "Initial deployment should return a URL"
  );
  assert.ok(
